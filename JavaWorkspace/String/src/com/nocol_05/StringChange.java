...
public class StringChange {
	public static void main(String[] args) {
		...
		// byte[] getBytes():把字符串转换为字节数组（返回字节数组）。
		byte[] bys = s.getBytes();  //byte[] bys 字节数组接收
		for (int x = 0; x < bys.length; x++) {
			System.out.println(bys[x]);
		}
		System.out.println("----------------");

		// char[] toCharArray():把字符串转换为字符数组（返回字符数组）。
		char[] chs = s.toCharArray();  //char[] ch 字符数组接收
		...
		System.out.println("s:" + s);  //JavaSE,装换后原来的不变,类似clone();
		// System.out.println("----------------");
